...
//  lab-3
//
//  Created by Kelly Brichta on 9/24/15.
...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    func updateTipTotals() {
        ...
        if numberOfPeople != nil {
            if numberOfPeople! > 1 {
                ...
                let alert=UIAlertController(title: "Warning", message: "The number of people must be greater than 1", preferredStyle: UIAlertControllerStyle.Alert)
                //create a UIAlertAction object for the button
                let cancelAction=UIAlertAction(title: "Cancel", style:UIAlertActionStyle.Cancel, handler: nil)
                alert.addAction(cancelAction) //adds the alert action to the alert object
                let okAction=UIAlertAction(title: "OK", style: UIAlertActionStyle.Default, handler: { action in self.people.text="2"
                    self.updateTipTotals()})
                alert.addAction(okAction)
                presentViewController(alert, animated: true, completion: nil)} //end else
    ...
    override func viewDidLoad() {checkAmount.delegate=self
